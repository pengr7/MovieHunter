...
public:
	rubik(GLfloat* p, GLfloat* d, GLfloat* c[], GLfloat l); //需要初始化Cube数组
	~rubik();
	GLfloat * getPosition();
	...
	void AllCubeDO(void (*a)(void)); //对所有Cube使用函数a
	//void RotatRubik(int xyz, int dir, GLfloat angle, int update);  
	//xyz = 0/1/2时对应围绕X/Y/Z轴转angle度，dir = 0/1为顺/逆时针,update为0不需更新Cube数组,1为需要更新。
